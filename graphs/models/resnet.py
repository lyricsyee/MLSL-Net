...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, 
        downsample=None, activation='relu'
    ):
        ...
        if activation == 'relu':
            ...
            raise Exception('Activation error! Should be [relu, lrelu]')
...
class ResNet(nn.Module):
    ...
    def load_pretrain(self, model_path):
        ...
        new_state_dict = {k: v for k, v in new_state_dict.items()
            if k in curmodel_dict
        }
        curmodel_dict.update(new_state_dict)
        self.load_state_dict(curmodel_dict)
